...
export const getDivisionNominations = async (req, res) => {
  ...
  const result = await Promise.all(
    nominations.map(async (n) => {
      ...
      // На витрину — только игроки с результатом. Где «больше — лучше», ноль и минус
      // не достижение: игрок без шайб в списке снайперов — пустое место, а не призёр.
      // Где «меньше — лучше», ноль как раз лучший результат (вратарь ничего не
      // пропустил, игрок без штрафа) — его оставляем. Пустая доля (у вратаря не было
      // бросков) не результат ни там, ни там. Свободные места сайт закрывает заглушками.
      const higherIsBetter = n.sort_order !== 'asc';
      players = players.filter((p) => p.value !== null && (!higherIsBetter || p.value > 0));
      return {
        id: n.id,
        name: n.name,
        scope: n.scope,
...
export const getTeamDetail = async (req, res) => {
  ...
  const rosterRes = await sharedPool.query(
    `SELECT
       ...
       -- на паре «человек + сезон» (user_season_consents): подписал раз — действует во всех
       -- его заявках сезона, в том числе после перехода в другую команду
       ...
       (usc.consent_url IS NOT NULL) AS has_consent,
    ...
    [tournamentTeamId, row.team_id, LEAGUE_ID, row.season_id]
  );
  ...
  const staffRes = await sharedPool.query(
    `SELECT
       ...
       (MAX(usc.consent_url) IS NOT NULL) AS has_consent,
       to_char(MAX(usc.consent_expires_at), 'YYYY-MM-DD') AS consent_expires_at
