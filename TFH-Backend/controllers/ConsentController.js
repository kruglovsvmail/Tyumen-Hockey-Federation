...
//      кладётся в бакет общей системы и прописывается в tournament_rosters.consent_url —
//      после чего документ виден в LMS, в кабинете команды и иконка на сайте зеленеет.
//
...
export const signConsent = async (req, res) => {
  ...
  try {
    ...
    // В общей базе лиги меняются ровно две колонки заявки: по ним LMS и кабинет команды
    // понимают, что документ есть (прав на что-либо ещё у сайта там нет и не должно быть).
    //
    // Порядок такой: открываем транзакцию в своей базе, пишем журнал, правим заявку
    // в общей базе и только после её успеха фиксируем журнал. Если на общую базу не
    // хватило прав или она недоступна, журнал откатывается — и в системе не остаётся
    // следов наполовину выполненной операции.
    ...
    try {
      ...
      // Две ошибки называем прямо: их чинит администратор, и «попробуйте позже» тут
      // только собьёт с толку — само оно не заработает никогда.
      const SETUP_ERRORS = {
        42501: 'У сайта нет прав на запись в базу лиги. Так подписание работать не будет — сообщите об этом администратору.',
