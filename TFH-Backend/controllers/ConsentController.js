...
//      кладётся в бакет общей системы и прописывается в user_league_consents — согласие
//      принадлежит паре «человек + лига», а не заявке, поэтому действует во всех заявках
//      игрока в лиге, в том числе после перехода в другую команду посреди сезона. После
...
const PERSON_QUERY = `
  ...
  -- Согласие лежит на паре «человек + лига»: одно на все заявки игрока в лиге
  LEFT JOIN user_league_consents ulc
         ON ulc.user_id = p.user_id AND ulc.league_id = s.league_id
  WHERE tt.id = $1
...
export const signConsent = async (req, res) => {
  ...
  try {
    ...
    // В общей базе лиги согласие ложится в user_league_consents (человек + лига): по нему
    // LMS и кабинет команды понимают, что документ есть, в любой заявке игрока в лиге.
    //
    ...
    // Согласие общее на лигу, так что прежний файл мог быть загружен под другой заявкой
